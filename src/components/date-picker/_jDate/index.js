...
class jDate {
    constructor(id, config = {}) {
        ...
        this.config.date.max = (config.date && config.date.max) || null;
        ...
        this.datas.time = this.datas.time.map((time) => {
            ...
            return Tools.fixTimeByStep(time, step);
        });
    ...
    initEventTimer() {
        ...
        inputDom.map((theInput, index) => {
            ...
            input.addEventListener('keyup', function (e) {
                ...
                time[index] = Tools.fixTimeByStep(values, step);
            ...
            document.addEventListener('mousemove', function (e) {
                ...
                if (self.config.time.type === jDate.Period) {
                    ...
                    if (index === 1 && time[0]) {
                        ...
                        if ((otherHours * 100 + parseInt(otherMinute)) > (tempTime[0] * 100 + tempTime[1])) {
                            tempTime = [otherHours, otherMinute];
                        }
                    }
                }
        ...
        this.doms.target.addEventListener('input', (e) => {
            ...
            if (this.config.time.type === jDate.Period) {
                ...
                if (times) {
                    ...
                    let sTime = Tools.fixTimeByStep([times[1], times[2]], step);
                    let eTime = Tools.fixTimeByStep([times[3], times[4]], step);
                    //
                    var finalTimes = [
                        sTime,
                        eTime
    ...
    chooseDate(date, e) {
        ...
        var isFit = this.datas.date.some((theDate, index) => {
            if (Tools.getDate(theDate).join('-') === Tools.getDate(date).join('-')) {
                fitIndex = index;
                return true;
            } else {
                return false;
                    ...
                    && this.config.date.max <= this.datas.date.length) {
                        ...
                        if (this.doms.target.parentNode.getElementsByTagName('*').length === 1) {
                            ...
                            var timer2 = setTimeout(() => {
                                this.doms.target.parentNode.removeChild(tips);
                                clearTimeout(timer1);
                                clearTimeout(timer2);
                            }, 2500);
                        }
    ...
    updateText(isInit) {
        ...
        if (datas.length >= 1) {
            ...
            switch (this.config.date.type) {
                ...
                case jDate.Multi:
                    if (datas.length == 2) {
                        var secondTime = Tools.getDate(datas[1]);
                        secondTime[1] = parseInt(secondTime[1], 10) + 1;
                        secondTime[1] = ('0' + secondTime[1]).slice(-2);
                        timeStr += firstTime.join('-') + '、' + secondTime.join('-');
                    } else if (datas.length > 2) {
                        timeStr += '已选择' + datas.length + '个日期';
                    } else {
                        timeStr += firstTime.join('-');
                    }
